...
void DoWorldOutline(inout vec3 color, float linearZ0) {
    ...
    for (int i = 0; i < sampleCount; i++) {
        ...
        outlined *= clamp(1.0 - ((depthCheckP + depthCheckN) - z * 2.0) * (32.0 * invZ), 0.0, 1.0);

        if (i <= 4) maxz = max(maxz, max(depthCheckP, depthCheckN));
        totalz += depthCheckP + depthCheckN;
    }

    float outlinea = 1.0 - clamp((z * 8.0 - totalz) * (64.0 * invZ), 0.0, 1.0) * clamp(1.0 - ((z * 8.0 - totalz) * 32.0 - 1.0) * invZ, 0.0, 1.0);
    float outlineb = clamp(1.0 + 8.0 * (z - maxz) * invZ, 0.0, 1.0);
    float outlinec = clamp(1.0 + 64.0 * (z - maxz) * invZ, 0.0, 1.0);
